...
import ${packageName}.dto.help.ExcelSaxReadCellDTO;
...
public class ExcelSheetHandler extends DefaultHandler {
	...
	private Constants.excelCellValueType type;

	private IExcelSaxRowRead read;

	private List<ExcelSaxReadCellDTO> rowList = Lists.newArrayList();	// 存储行记录的容器
	...
	public void startElement(String uri, String localName, String name, Attributes attributes) throws SAXException {
		...
		if ("c".equals(name)) {
			...
			int nowColIndex = ExcelUtil.getColumnIndexByCellName(rowId);
			// 注意，lastCellId在上一行结束的时候都会置空
			if (lastCellId != null) {
				// 该行非第一个有记录的单元格（index从1开始计）
				int lastColIndex = ExcelUtil.getColumnIndexByCellName(lastCellId);
				// 计算同一行两个单元格之间的间隔
				int gap = nowColIndex - lastColIndex;
				for (int i = 0; i < gap - 1; i++) {
					// 间隔大于1，说明之间有空单元格，追加（gap - 1）条记录
					rowList.add(curCol, new ExcelSaxReadCellDTO(Constants.excelCellValueType.String, null));
					curCol++;
				}
			} else {
			...
			if ("s".equals(cellType)) {
				type = Constants.excelCellValueType.String;
				return;
			}
			// 日期格式
			cellType = attributes.getValue("s");
			if ("1".equals(cellType)) {
				type = Constants.excelCellValueType.Date;
			} else if ("2".equals(cellType)) {
				type = Constants.excelCellValueType.Number;
			}
		} else if ("t".equals(name)) {
			// 当元素为t时
			type = Constants.excelCellValueType.TElement;
		}
	}
	...
	public void endElement(String uri, String localName, String name) {
		...
		if (Constants.excelCellValueType.String.equals(type)) {
			try {
		...
		if (Constants.excelCellValueType.TElement.equals(type)) {
			// t元素也包含字符串
			String value = lastContents.trim();
			rowList.add(curCol, new ExcelSaxReadCellDTO(Constants.excelCellValueType.String, value));
			curCol++;
			type = Constants.excelCellValueType.None;
			...
			if (Constants.excelCellValueType.Date.equals(type)) {
				Date date = HSSFDateUtil.getJavaDate(Double.valueOf(value));
				rowList.add(curCol, new ExcelSaxReadCellDTO(Constants.excelCellValueType.Date, date));
			} else if (Constants.excelCellValueType.Number.equals(type)) {
				BigDecimal bd = new BigDecimal(value);
				rowList.add(curCol, new ExcelSaxReadCellDTO(Constants.excelCellValueType.Number, bd));
			} else if (Constants.excelCellValueType.String.equals(type)) {
				rowList.add(curCol, new ExcelSaxReadCellDTO(Constants.excelCellValueType.String, value));
			}
			curCol++;
